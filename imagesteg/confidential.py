...
def Encode():
    Screen.destroy()
    EncScreen = Tk()
    EncScreen.title("Encode-Sahithi")
...
def Decode():
    Screen.destroy()
    DecScreen = Tk()
    DecScreen.title("Decode-Sahithi")
    DecScreen.geometry("500x500+300+300")
    DecScreen.config(bg="Yellow")
    ...
    SelectButton = Button(text="Select the file",command=OpenFile)
    SelectButton.place(relx=0.4,rely=0.4)
    EncodeButton=Button(text="Decode",command=Decoder)
    EncodeButton.place(relx=0.4,rely=0.5)

Screen = Tk()
Screen.title("Image Steganography by - Sahithi ")
